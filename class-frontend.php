...
//ADD XFBML
add_filter('language_attributes', 'fbcp_schema');
function fbcp_schema($attr) {
...
function fbcphortcode($fbatts) {
    extract(shortcode_atts(array(
		"fbcp" => get_option('fbcp'),
		"url" => get_permalink(),
    ), $fbatts));
    if (!empty($fbatts)) {
	...
	if (!isset($fbcp['count'])) {$fbcp['count'] = "";}
		if ($fbcp['count'] == 'on') {
			if ($fbcp['countstyle'] == '') {
				$commentcount = "<p>";
			} else {
				$commentcount = "<p class=\"".$fbcp['countstyle']."\">";
			}
			$commentcount .= "<fb:comments-count href=".$url."></fb:comments-count> ".$fbcp['countmsg']."</p>";
		}
		if ($fbcp['title'] != '') {
			if ($fbcp['titleclass'] == '') {
				$commenttitle = "<h3>";
			} else {
				$commenttitle = "<h3 class=\"".$fbcp['titleclass']."\">";
			}
			$commenttitle .= $fbcp['title']."</h3>";
		}
		if (!isset($commentcount)) {$commentcount = "";}
		$fbcommentbox = "<!-- Facebook Comments Plugin for WordPress: # -->".$commenttitle.$commentcount;

      	if ($fbcp['html5'] == 'on') {
			$fbcommentbox .=	"<div class=\"fb-comments\" data-href=\"".$url."\" data-num-posts=\"".$fbcp['num']."\" data-width=\"".$fbcp['width']."\" data-colorscheme=\"".$fbcp['scheme']."\"></div>";

    } else {
    $fbcommentbox .= "<fb:comments href=\"".$url."\" num_posts=\"".$fbcp['num']."\" width=\"".$fbcp['width']."\" colorscheme=\"".$fbcp['scheme']."\"></fb:comments>";
     }
...
add_shortcode('fbcp', 'fbcphortcode');
