<?php
define("FBC_NAME","Facebook Comments Plus");
define("FBC_TAGLINE","Adds Facebook Comments Plus");
define("FBC_URL","https://www.google.com/+tayyabismail0o1");
define("FBC_EXTEND_URL","#");
define("FBC_AUTHOR_TWITTER","#");
define("FBC_DONATE_LINK","#");

add_action('admin_init', 'fbcp_init' );
function fbcp_init(){
	register_setting( 'fbcp_options', 'fbcp' );
	$new_options = array(
		'fbml' => 'on',
		'old_sdk' => '',
		'opengraph' => 'off',
	...
	foreach( $new_options as $key => $value ) {
		if( $existing = get_option( 'fbcp_' . $key ) ) {
			$new_options[$key] = $existing;
			delete_option( 'fbcp_' . $key );
		}

	}


	add_option( 'fbcp', $new_options );
}


add_action('admin_menu', 'show_fbcp_options');
function show_fbcp_options() {
	add_options_page('Facebook Comments Plus Settings', 'FB Comments Plus', 'manage_options', 'fbcp', 'fbcp_options');
}


function fbcp_fetch_rss_feed() {
...
function fbcp_admin_notice(){
$options = get_option('fbcp');
if ($options['appID']=="") {
	$fbadminurl = get_admin_url()."options-general.php?page=fbcp";
    echo '<div class="error">
       <p>Please enter your Facebook App ID for Facebook Comments Plus to work properly. <a href="'.$fbadminurl.'"><input type="submit" value="Enter App ID" class="button-secondary" /></a></p>
    </div>';
}
}
add_action('admin_notices', 'fbcp_admin_notice');

// ADMIN PAGE
function fbcp_options() {
...
?>
    ...
    <div class="pea_admin_wrap">
		...
		<form method="post" action="options.php" id="options">
			<?php settings_fields('fbcp_options'); ?>
			<?php $options = get_option('fbcp'); 
			...
			<table class="form-table">
				<tr valign="top"><th scope="row"><a href="https://developers.facebook.com/apps" style="text-decoration:none" target="_blank">Create an App to handle your comments</a></th>
					<td><small style="font-size:12px;">Click <strong>'Create New App</strong>' to the top right of the page. Name the App something memorable e.g. "Comments". <small style="font-size:12px;">Ensure you enter <strong><?php echo $domain; ?></strong> in both "App Domains" and in the url fields.</small> Once you have done it enter app id here and press enter:</small><br><strong>APP ID: </strong><input id="appID" type="text" name="fbcp[appID]" value="<?php echo $options['appID']; ?>" /><br><br>
</td>
				</tr>
			</table>
</div>
...
<?php } ?>
			...
			<table class="form-table">
				...
				<tr valign="top"><th scope="row"><label for="appID">Moderators</label></th>
					<td><input id="mods" type="text" name="fbcp[mods]" value="<?php echo $options['mods']; ?>" size="50" /><br><small style="font-size:12px;">By default, all admins to the App ID can moderate comments. To add moderators, enter each Facebook Profile ID by a comma <strong>without spaces</strong>. To find your Facebook User ID, click <a href="https://developers.facebook.com/tools/explorer/?method=GET&path=me" target="blank">here</a> where you will see your own. To view someone else's, replace "me" with their username in the input provided or select your app from above.If not working use <a href="http://ipics32.blogspot.com/p/get-your-facebook-id.html" target="blank">this</a>.</small></td>
				</tr>
			</table>
...
<?php if ($options['appID']!="") { ?>
				<tr valign="top"><th scope="row"><label for="appID">Facebook App ID</label></th>
					<td><input id="appID" type="text" name="fbcp[appID]" value="<?php echo $options['appID']; ?>" /></td>
				</tr>
<?php } ?>
				<tr valign="top"><th scope="row"><label for="fbml">Enable FBML</label></th>
					<td><input id="fbml" name="fbcp[fbml]" type="checkbox" value="on" <?php checked('on', $options['fbml']); ?> /> <small style="font-size:12px;">Only disable this if you already have XFBML enabled elsewhere in your theme/plugins.</small></td>
				<tr valign="top"><th scope="row"><label for="fbns">Use Facebook NameServer</label></th>
					<td><input id="fbns" name="fbcp[fbns]" type="checkbox" value="on" <?php checked('on', $options['fbml']); ?> /> <small style="font-size:12px;">Only enable this if Facebook Comments Plus do not appear.</small></td>
				</tr>
				<tr valign="top"><th scope="row"><label for="opengraph">Use Open Graph NameServer</label></th>
					<td><input id="opengraph" name="fbcp[opengraph]" type="checkbox" value="on" <?php checked('on', $options['opengraph']); ?> /> <small style="font-size:12px;">Only enable this if Facebook Comments Plus are not appearing, not all information is being passed to Facebook or if you have not enabled Open Graph elsewhere within you website.</small></td>
				</tr>
				<tr valign="top"><th scope="row"><label for="html5">Use HTML5</label></th>
					<td><input id="html5" name="fbcp[html5]" type="checkbox" value="on" <?php checked('on', $options['html5']); ?> /></td>
				</tr>
				<tr valign="top"><th scope="row"><label for="linklove">Credit</label></th>
					<td><input id="credit" name="fbcp[linklove]" type="checkbox" value="on" <?php checked('on', $options['linklove']); ?> /></td>
				</tr>
			</table>
			...
			<table class="form-table">
				<tr valign="top"><th scope="row"><label for="posts">Singular Posts</label></th>
					<td><input id="posts" name="fbcp[posts]" type="checkbox" value="on" <?php checked('on', $options['posts']); ?> /> <small style="font-size:12px;">This includes all posts, custom post types and attacments. Note that you can disable Facebook Comments Plus on a post using the Facebook Comments Plus checkbox present on the post editor page.</small></td>
				</tr>
				<tr valign="top"><th scope="row"><label for="pages">Pages</label></th>
					<td><input id="pages" name="fbcp[pages]" type="checkbox" value="on" <?php checked('on', $options['pages']); ?> /> <small style="font-size:12px;">Note that you can disable Facebook Comments Plus on a page using the Facebook Comments Plus checkbox present on the page editor page.</small></td>
				</tr>
				<tr valign="top"><th scope="row"><label for="homepage">Homepage</label></th>
					<td><input id="home" name="fbcp[homepage]" type="checkbox" value="on" <?php checked('on', $options['homepage']); ?> /></td>
				</tr>
				<tr valign="top"><th scope="row"><label for="language">Language</label></th>
					<td>
						<select name="fbcp[language]">
				...
				<tr valign="top"><th scope="row"><label for="num">Number of Comments</label></th>
					<td><input id="num" type="text" name="fbcp[num]" value="<?php echo $options['num']; ?>" /> <small style="font-size:12px;">Default is <strong>5</strong>.</small></td>
				</tr>
				<tr valign="top"><th scope="row"><label for="width">Width</label></th>
					<td><input id="width" type="text" name="fbcp[width]" value="<?php echo $options['width']; ?>" /> <small style="font-size:12px;">Default is <strong>100%</strong>. Keep this to ensure that the comment box is responsive.</small></td>
				</tr>
				<tr valign="top"><th scope="row"><label for="title">Title</label></th>
					<td><input id="title" type="text" name="fbcp[title]" value="<?php echo $options['title']; ?>" />  <input type="text" name="fbcp[titleclass]" value="<?php echo $options['titleclass']; ?>" /> <small style="font-size:12px;">Add a CSS class here to give style according to it.</small></td>
				</tr>
			...
			<h3 class="title">Using the Shortcode</h3>
              <table class="form-table">
				<tr valign="top"><td>
<small style="font-size:12px;"><p>Above settings are for automatic insertion of the Facebook Comments Plus box.</p>
...
<p><input style="width:104px;" type="text" value="[fbcp]" onClick="this.setSelectionRange(0, this.value.length);"> 
<p>To enter the shortcode directly into templates using PHP, enter folowing code
<p><input style="width:250px;" type="text"  onClick="this.setSelectionRange(0, this.value.length);" name="code_snippet" value="<?php echo("<?php echo do_shortcode('[fbcp]'); ?>"); ?>"></p>
</p></small>
				</td>
				</tr>
			</table>
...
<div style="text-align: center;">
                                    ...
                                    <div style="margin-top:10px; margin-bottom: 8px;">
 <a href="https://wordpress.org/plugins/new-facebook-comments/" target="_blank" style="width:90%; margin-top:5px; margin-bottom: 5px; "><strong>Facebook Comments Plus</strong></a>
 Allows you to add Facebook Comments Plus system in your website easily.
<div style="text-align: center;">
    ...
    foreach ( $post_types as $post_type ) {
        if ( "post" == $post_type ) {
        	if ($options['posts']=='on') {
	            add_meta_box(
	                'fbc_sectionid',
	                __( 'Facebook Comments Plus', 'fbc_singlemeta' ),
	                'fbc_metabox',
	                $post_type,
	                'advanced',
	                'core'
...
?>
    <input id="_disable_fbc" name="_disable_fbc" type="checkbox" value="on" <?php checked('on', $_disable_fbc); ?> /> <label for="_disable_fbc"><strong>DISABLE</strong> Facebook Comments Plus</label></td>
<?php
}
